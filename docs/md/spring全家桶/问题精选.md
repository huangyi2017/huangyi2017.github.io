...
'prepareRefresh,obtainRefreshBeanFactory（创建bean工厂，扫描配置解析成beanDefinition注册到bean工厂）,prepareBeanFactory(设置bean的类加载器，添加beanProcessProcessor）,
...
registerBeanPostProcessor（注册beanPostProcessor），finishBeanFactoryInitialization（单例bean创建和初始化），finishRefresh（缓存清理，容器初始化事件发布）'?'正确':'错误')">
submit</button>
...
<textarea rows="5" cols="50" id="spring20"></textarea>
<button onclick="alert(document.getElementById('spring20').value==
'BeanDefinition信息的Resource的定位（封装成resource）、载入（读取resource）、解析（将resource解析成BeanDefinition）、注册（调用BeanDefinitionRegistry注册到IOC容器）'?'正确':'错误')">
submit</button>
