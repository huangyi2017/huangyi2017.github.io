...
'运行SpringApplication.run()方法，构造SpringApplication对象（确定应用程序类型，加载初始化器，加载监听器，推断程序运行主类），创建和配置环境，创建ApplicationContext对象，准备上下文（ioc容器创建），发布启动和就绪事件'?'正确':'错误')">
submit</button>
...
'对象的创建和依赖关系的管理都交由Spring容器来完成，而不是由对象自己来完成,它的核心是依赖注入，通过依赖注入来实现对象的创建和依赖关系的自动装配'?'正确':'错误')">
submit</button>
