...
'当一个Bean被注入到容器中时，容器会检查该Bean是否有被@PostConstruct注解标记的方法。如果有，容器会在该Bean的所有依赖注入完成之后，自动调用该方法'?'正确':'错误')">
submit</button>

31：Bean的初始化如何工作<p/>
<textarea rows="5" cols="50" id="spring31"></textarea>
<button onclick="alert(document.getElementById('spring31').value==
'bean初始化后，判断是否实现InilizationBean接口，如有，调用afterPropertiesSet方法'?'正确':'错误')">
submit</button>
...
<textarea rows="5" cols="50" id="spring29"></textarea>
<button onclick="alert(document.getElementById('spring29').value==
'1：autowired是spring注解，resource是javaEE;2:autowired按类型注入，resouce按名称注入;3：autorire可用于构造器，resouce不可以'?'正确':'错误')">
submit</button>