...
'一款断路器，防止级联失败，通过资源隔离，监控，降级策略等机制，实现容错。Hystrix会监控外部服务调用，当某服务失败率或超时率超过阀值，则进入停止访问该服务，一段时间后，再尝试小量请求，如成功，则恢复，不成功则继续熔断'?'正确':'错误')">
submit</button>
...
'1：追随者发现leader失联后，发起新的选举，将自己身份转为候选者；2：如遇多个候选者，且遇平票情况，raft协议对每一个候选人都设置了随机超时时间；
