...
<button onclick="alert(document.getElementById('cache9').value=='一种轻量级广播消息机制，发布者发送消息给指定通道，订阅者订阅指定通道，不做持久化和ack。应用场景：异步处理，业务解耦，分布式锁'?'正确':'错误')">submit</button>
...
<button onclick="alert(document.getElementById('cache11').value=='used_memory，maxmemory，connected_clients，ops_per_sec，total_connections_received，total_commands_processed'?'正确':'错误')">submit</button>

12:Redis 的集群模式是什么？如何实现？<p/>
<textarea rows="5" cols="50" id="cache12">
</textarea>
<button onclick="alert(document.getElementById('cache12').value=='集群模式自动将数据进行分片，每个master上放一部分数据。实现：一个集群中总共有16384个节点，集群会将这16384个节点平均分配给每个节点，再将key使用CRC16算法计算出散列值，然后与16384取余数，这样的就可以使每个键能够尽量的均匀分布在16384个插槽中'?'正确':'错误')">submit</button>

13:Redis 的主从复制模式下，主从节点如何保持数据同步？<p/>
<textarea rows="5" cols="50" id="cache13">
</textarea>
<button onclick="alert(document.getElementById('cache13').value=='1：从库执行replicaof并发送psync命令给主库；2：主库收到命令后将内存快照保存为RDB文件，并将RDB文件发送给从库；3：从库收到RDB文件，清空当前数据库，加载RDB文件到内存'?'正确':'错误')">submit</button>

14:Redis 的内存管理机制是什么？如何优化 Redis 的内存使用？<p/>
<textarea rows="5" cols="50" id="cache14">
</textarea>
<button onclick="alert(document.getElementById('cache14').value=='内存管理：设置最大内存，内存回收。优化：缩减键和值的长度，共享对象池，字符串优化（SDS），编码优化，减少键的数量'?'正确':'错误')">submit</button>

15:Redis 的性能瓶颈是什么？如何优化性能？<p/>
<textarea rows="5" cols="50" id="cache15"></textarea>
<button onclick="alert(document.getElementById('cache15').value=='大key查询，持久化开销，网络延时，优化：拆分大key，数据压缩，合理过期时间，合理的内存淘汰策略，合理的持久化策略，合并指令，采用集群模式等'?'正确':'错误')">submit</button>

16:Redis 的内存淘汰策略有哪些？如何选择合适的策略？<p/>
<textarea rows="5" cols="50" id="cache16"></textarea>
<button 
onclick="alert(document.getElementById('cache16').value=='volatile-lru,volatile-lfu,volatile-random,volatile-ttl,
...
19:Redis 如何对数据进行分片？有哪些常用的分片策略？<p/>
<textarea rows="5" cols="50" id="cache18"></textarea>
<button onclick="alert(document.getElementById('cache18').value==
'把数据划分成多份，每一份用一个实例来保存。分片策略：逻辑拆分，哈希取模，范围切片，一致性hash'?'正确':'错误')">submit</button>

20:Redis 如何处理并发访问冲突，有哪些常用的并发控制方法？<p/>
