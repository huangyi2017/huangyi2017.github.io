...
'used_memory，maxmemory，connected_clients，ops_per_sec，total_connections_received，total_commands_processed，client_output_limit_buffer'?'正确':'错误')">
submit</button>
...
'设置最大内存，内存回收。优化：缩减键和值的长度，共享对象池，字符串优化（SDS），编码优化，减少键的数量（hash)'?'正确':'错误')">submit</button>
...
'CPU瓶颈，内存瓶颈，I/O瓶颈，Redis命令本身的瓶颈，网络瓶颈。优化：数据压缩，合理过期时间，合理的内存淘汰策略，合理的持久化策略，拆分大key，合并指令，集群模式等'?'正确':'错误')">
submit</button>
