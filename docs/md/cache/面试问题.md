...
<button onclick="alert(document.getElementById('cache11').value=='used_memory，maxmemory，connected_clients，ops_per_sec，total_connections_received，total_commands_processed，client_output_limit_buffer'?'正确':'错误')">submit</button>

12:Redis 的集群模式是什么？如何实现？<p/>
<textarea rows="5" cols="50" id="cache12">
</textarea>
<button onclick="alert(document.getElementById('cache12').value=='集群模式是自动将数据进行分片，每个master上存储一部分数据。实现：一个集群总共有16384个插槽，集群会将这16384个节点平均分配给每个节点，再将key-value使用CRC16算法计算出散列值，然后除16384取余，这样就可以使所有数据能够均匀分布在16384个插槽中'?'正确':'错误')">submit</button>
...
<button onclick="alert(document.getElementById('cache14').value=='内存管理机制：设置最大内存，内存回收。优化：缩减键和值的长度，共享对象池，字符串优化（SDS），编码优化，减少键的数量（hash)'?'正确':'错误')">submit</button>

15:Redis 的性能瓶颈是什么？如何优化性能？<p/>
<textarea rows="5" cols="50" id="cache15"></textarea>
<button onclick="alert(document.getElementById('cache15').value=='大key查询，持久化开销，网络延时。优化：拆分大key，数据压缩，合理过期时间，合理的内存淘汰策略，合理的持久化策略，合并指令，集群模式等'?'正确':'错误')">submit</button>
